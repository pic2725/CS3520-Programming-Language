...
(define (interp-expr [e : Exp]) : S-Exp
  (type-case Value (interp e mt-env)
    ...
    [(pairV fist second) `pair]))


;; helper method
(define (pairHelper [fst? : Boolean] [e : Exp] [env : Env])
  [type-case Value (interp e env)
    [(pairV fst snd) (if fst? (force fst)
                              (force snd))]
    [else (error 'interp "wrong pair")]])
...
(define (interp [a : Exp] [env : Env]) : Value
  (type-case Exp a
    ...
    [(pairE fst snd) (pairV (delay fst env (box (none)))
                            (delay snd env (box (none))))]
    
    [(fstE fst) (pairHelper #t fst env)]
    [(sndE snd) (pairHelper #f snd env)]
...
(module+ test
  ...
  (test/exn (interp (parse `x) mt-env)
            "free variable")
  (test/exn (num-zero? (closV 'x (numE 2) mt-env))
            "not a number")
  ...
  (test (interp-expr (parse `{fst {pair 1 2}}))
        `1)
  (test/exn (pairHelper #f (parse `10) mt-env)
            "wrong pair")
  ...
  (test/exn (num-zero? (closV 'x (numE 4) mt-env))
            "not a number")
  (test (interp-expr (parse `{lambda {r} 1}))
        `function)
