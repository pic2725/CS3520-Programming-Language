...
(define-type Exp
  ...
  (selectE [firstExp : Exp]
           [secondExp : Exp])
  
  (instanceofE [firstExp : Exp]
               [symbol : Symbol])

  )

(define-type Class
  (classC [field-names : (Listof Symbol)]    ;;make it list
          [methods : (Listof (Symbol * Exp))]
          [superclasses : (Listof Symbol)])) ;; add list of superclasses
...
(define (classFinder [list : (Listof (Symbol * Class))] [symbol : Symbol]) : Class
  (if (symbol=? 'Object symbol)
      (classC empty empty empty)
      (type-case (Listof (Symbol * Class)) list
        [empty
         (error 'find (string-append "not found: " (symbol->string symbol)))]
        [(cons p rst-l)
         (if (symbol=? (fst p) symbol)
             (snd p)
             (find rst-l symbol))])))
...
(module+ test
  ...
  (test/exn (find (list (values 'a 1)) 'x)
            "not found: x")
  (test (classFinder (list (values 'a (classC empty empty empty))) 'a)
        (classC empty empty empty))
  (test (classFinder (list (values 'a (classC empty empty empty))) 'Object)
        (classC empty empty empty))
  (test/exn (classFinder empty 'a)
            "not found: a"))
    ...
    (local [(define (recur expr)
      ...
      (type-case Exp a
        ...
        [(newE class-name field-exprs)
         (local [(define c (classFinder classes class-name))
        ...
        [(getE obj-expr field-name)
         (type-case Value (recur obj-expr)
           [(objV class-name field-vals)
            (type-case Class (classFinder classes class-name)
              [(classC field-names methods superclasses)
        ...
        [(selectE firstExp secondExp)
         (type-case Value (recur firstExp)
           [(numV number) (local [(define obj (recur secondExp))]
                            (type-case Value obj
                              [(objV class-name field-values)
                               (local [(define c (classFinder classes class-name))]
                                 (if (zero? number)
                                     (checkC (classC-methods c) 'zero class-name classes obj)
                                     (checkC (classC-methods c) 'nonzero class-name classes obj)
                                     ))]
                              (else (error 'interp "not an object"))))]
           [else (error 'interp "not a number")])]

        [(instanceofE firstExp symbol)
         (type-case Value (recur firstExp)
           [(objV class-name field-vals) (local [(define temp (classFinder classes class-name))]
                                           (if
                                            (or (symbol=? symbol class-name)
                                                   (member symbol (classC-superclasses temp)))
                                               (numV 0)
                                               (numV 1)))]
           [else (error 'interp "not an object")])]
...
(define (checkC [l :  (Listof (Symbol * Exp))] [name : Symbol] [class-name : Symbol] [classes : (Listof (Symbol * Class))] [obj : Value] )
  (type-case (Listof (Symbol * Exp)) l
    [empty (error 'interp "empty")]
    [(cons p rst-l) (if(symbol=? (fst p) name)
...
(define-type ExpI
  ...
  (selectI [firstExp : ExpI]
           [secondExp : ExpI])

  (instanceofI [firstExp : ExpI]
               [symbol : Symbol])
    ...
    (type-case ExpI a
      ...
      [(superI method-name arg-expr)
       (ssendE (thisE)
               ...
               method-name
               (recur arg-expr))]
      [(selectI cond obj) (selectE (recur cond) (recur obj))]
      [(instanceofI obj super) (instanceofE (recur obj) super)]
...
(module+ test
  ...
  (test/exn(interp(selectE (numE 0) (numE 5))
    (list
     (values 'Snowball (classC '(size) (list
                                        (values 'zero (thisE)) (values 'nonzero (newE 'Snowball (list
                                                                                                 (plusE(numE 1)(getE (thisE) 'size))))))(list 'Object))))
    (objV 'Object '())
    (numV 0))
   "not an object")

  (test/exn
   (interp(getE (selectE (thisE) (newE 'Snowball (list (numE 0)))) 'size)
    (list
     (values 'Snowball (classC '(size) (list
                                        (values 'zero (thisE)) (values 'nonzero (newE 'Snowball (list
                                                                                                 (plusE(numE 1)(getE (thisE) 'size))))))(list 'Object))))
  ...
  (test/exn
   (interp (instanceofE (numE 0) 'Object)
    (list
     (values 'Snowball (classC '(size) (list
                                        (values 'zero (thisE)) (values 'nonzero (newE 'Snowball (list
                                                                                                 (plusE(numE 1)(getE (thisE) 'size))))))(list 'Object))))
  ...
  (type-case ClassI c
    [(classI super-name field-names methods)
     (classC
      ...
      (map (lambda (m)
             (values (fst m)
                     (exp-i->c (snd m) (first super-name))))
           methods)
      super-name)]))
...
(define (flatten-class [name : Symbol]
  ...
  (type-case Class (classFinder classes-not-flat name)
    [(classC field-names methods superclasses)
     (type-case Class (flatten-super name classes-not-flat i-classes)
       [(classC super-field-names super-methods super-superclasses)
        (classC
         (add-fields super-field-names field-names)
         (add/replace-methods super-methods methods)
         (cons
          (first superclasses) super-superclasses))])]))
...
(define (flatten-super [name : Symbol]
  ...
  (type-case ClassI (find i-classes name)
    [(classI super-names field-names i-methods)
     (let ([super-name (first super-names)])
